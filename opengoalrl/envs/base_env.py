...
def _patch_grf_api(env):
    """Fix gym old/new API mismatch inside gfootball's wrapper chain.

    Older ``FootballEnv`` returns single obs from ``reset()`` and a 4-tuple
    from ``step()``, but newer gym wrappers expect ``(obs, info)`` and
    ``(obs, reward, terminated, truncated, info)``.  Walk the entire chain
    and patch both methods at every level.
    """
    current = env
    while True:
